...
INSERT INTO Articulo (ArticuloId, Nombre, Descripcion, MedidaId) 
VALUES (2, 'Disco Duro Externo', 'Disco duro externo de 1TB para copias de seguridad', 1);

insert into Compra values (4, '22-8-2024')
insert into CompraDetalle values (4,2, 'Jetstereo',10,3000,0.15)

insert into Factura values(3,'16-8-2024','P',1)
insert into Factura values(4,'16-8-2024','P',1)
insert into FDArticulo values (3,2,3500,0.15,5,1)
insert into FDArticulo values (4,2,3500,0.15,4,1)

insert into Inventario values (2,10,1,10.00)

exec spInventario 2

create or alter view vInventario 
as
   select distinct i.ArticuloId, a.Nombre as Articulo, a.Descripcion, m.Nombre as Medida, 
   i.Existencia, cd.Proveedor , round(i.Rentabilidad,2) as rentabilidad, a.Estado
   from Inventario as i 
   inner join Articulo as a on i.ArticuloId = a.ArticuloId inner join Medida as m on m.MedidaId = i.Medidaid
   inner join CompraDetalle as cd on cd.Articuloid = i.ArticuloId
   where a.Estado = 'V'
go

create or alter view vInventarioD
as
   select distinct i.ArticuloId, a.Nombre as Articulo, a.Descripcion, m.Nombre as Medida, 
   i.Existencia, cd.Proveedor , round(i.Rentabilidad,2) as rentabilidad, a.Estado
   from Inventario as i 
...
alter table Articulo add Estado VARCHAR(1) not null;

create or alter procedure spEliminarInv @ArticuloID int 
as
   update Articulo set Estado = 'D' where ArticuloId = @ArticuloID
go

create or alter procedure spEditarInv @ArticuloID int, @NombreArt varchar(100), @Desc varchar(100), @Prov varchar (100), @Estado varchar(1)
as

   update Articulo set Nombre = @NombreArt where ArticuloId = @ArticuloID;
   update Articulo set Descripcion = @Desc where ArticuloId = @ArticuloID;
   update Articulo set Estado = @Estado where ArticuloId = @ArticuloID
   update CompraDetalle set Proveedor = @Prov where ArticuloId = @ArticuloID;
